...
class HIDDevice:
    def __init__(self, device: dict, loop: asyncio.AbstractEventLoop):
        ...
        self.hidraw_file = os.open('/dev/' + device["hidraw"], os.O_RDWR | os.O_NONBLOCK)
    ...
    def hidraw_event(self):
        ...
        try:
            ...
            self.logger.error(f"HID device {self.device_id} exception on read. closing")
            return
        ...
        for device in os.listdir('/sys/bus/hid/devices'):
            try:
                with open('/sys/bus/hid/devices/' + device + '/uevent', 'r') as uevent:
                    ...
                    self.capturing_devices[device] = HIDDevice({"instance": device, "hidraw": hidraw}, self.loop)
